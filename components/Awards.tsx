...
export default function Awards() {
  ...
  return (
    <section id="pages" className="py-20 lg:py-32 px-6 lg:px-12 bg-background clip-diagonal reveal relative overflow-hidden">
      <div className="max-w-screen-2xl mx-auto">
        <div className="flex items-center gap-4 mb-10">
          <label className="font-mono text-[9px] md:text-xs uppercase tracking-[0.4em] text-lime block font-bold">
            [PROUD MOMENTS]
          </label>
          <span className="font-pinyon text-xl md:text-3xl text-foreground/30 lowercase mt-1">
            dream that achievs
          </span>
        </div>

        <h2 className="text-5xl md:text-8xl mb-24 leading-[0.9] tracking-tight text-foreground max-w-4xl uppercase">
        ...
        <div className="flex flex-col border-t border-border">
          {awards.map((award, index) => (
            <div
              ...
              className="group relative flex flex-col md:flex-row items-start md:items-center justify-between py-10 md:py-16 border-b border-border transition-all duration-300 hover:bg-foreground/[0.02]"
            >
              {/* Year */}
              ...
              <div className="flex-1 md:px-20">
                <div className="relative h-[48px] md:h-[72px] overflow-hidden [perspective:1000px] [transform-style:preserve-3d]">
                  <h3 className="text-4xl md:text-6xl text-foreground font-bebas tracking-tight transition-all duration-700 ease-[cubic-bezier(0.76,0,0.24,1)] group-hover:-translate-y-full group-hover:rotate-x-[-90deg] origin-top select-none">
                    {award.title}
                  </h3>
              ...
              {/* Category */}
              <div className="font-mono text-foreground/30 text-[10px] md:text-xs uppercase tracking-widest transition-colors group-hover:text-foreground/60">
                {award.category}
              </div>
